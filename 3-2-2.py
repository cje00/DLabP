...
학생들의 이번 학기 성적을 담고 있는 리스트 current_grade,
직전 학기 성적을 담고 있는 리스트 last_grade가 매개변수로 주어질 때,
...
학생들의 이번 학기 성적, 지난 학기 성적이 들어있는 리스트
current_grade, last_grade가 solution 함수의 매개변수로 주어집니다.
* current_grade의 길이와 last_grade의 길이는 같으며,
길이는 1 이상 200 이하입니다.
...
def solution(current_grade, last_grade):
    rank = func_b(current_grade)  # rank = func_@@@(@@@)
    max_diff_grade = func_c(current_grade, last_grade)  # max_diff_grade = func_@@@(@@@)
    answer = func_a(current_grade, last_grade, rank, max_diff_grade)  # answer = func_@@@(@@@)
    return answer
