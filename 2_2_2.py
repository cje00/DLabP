...
'''
#문제2
자연수가 들어있는 리스트에 3의 배수와 5의 배수 중
어떤 수가 더 많은지 알아보려 합니다.
...
리스트에 3의 배수의 개수가 더 많다면 "three"를,
5의 배수의 개수가 더 많다면 "five"를,
3의 배수와 5의 배수의 개수가 같다면 "same"을 return 하도록
...
def solution(arr):
    count_three = func_c(arr)  # count_three = func_@@@(@@@)
    
    count_five = func_a(arr)  # count_five = func_@@@(@@@)
    
    answer = func_b(count_three, count_five) # answer = func_@@@(@@@)
